...
class AttentionMessagePassing(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        alpha = self.attention(
            torch.cat([x_proj[row], x_proj[col], edge_proj], dim=-1)
        ).squeeze(
            -1
        )  # Shape: [num_edges]
        alpha = torch_scatter.scatter_softmax(alpha, col)  # Per target node softmax
        ...
        weighted_messages = alpha.unsqueeze(-1) * messages
        output = torch.zeros_like(x)
        output = output.index_add(0, col, weighted_messages)

        return output


class GNNLayer(nn.Module):
    def __init__(self, node_dim, edge_dim, hidden_dim, out_dim, use_gru=False):
...
class GNN(nn.Module):
    def __init__(
        self, node_dim, edge_dim, hidden_dim, num_layers, out_dim, use_gru=False
    ):
        ...
        self.layers = nn.ModuleList(
            [
                GNNLayer(
                    node_dim, edge_dim, hidden_dim, out_dim=node_dim, use_gru=use_gru
                )
                for _ in range(num_layers)
            ]
        )
        self.layers[-1] = GNNLayer(
            node_dim, edge_dim, hidden_dim, out_dim, use_gru=use_gru
        )
